...
_CJK_CLASS = r"[\u4e00-\u9fa5\u3000-\u303f\uff01-\uff5e]"
_RE_QUOTE_PUNC = re.compile(r'([""\'\'])\s*([,.?!:;])\s*')
_RE_PUNC_QUOTE = re.compile(r'([,.?!:;])\s*([""\'\'])')
...
def is_near_chinese(text, pos):
    ...
    while j < len(text) and text[j] in (" ", "\t"):
        j += 1
    if j < len(text) and is_cjk(text[j]):
        return True
    return False
        ...
        for _ in range(2):
            for eng_punc, chn_punc in _PUNCTUATION_MAP.items():
                ...
                protected_line = re.sub(
                    f"({_CJK_CLASS})\\s*{esc}\\s*", f"\\1{chn_punc}", protected_line
                )
                protected_line = re.sub(
                    f"{esc}\\s*({_CJK_CLASS})", f"{chn_punc}\\1", protected_line
                )

            protected_line = _RE_QUOTE_PUNC.sub(
                lambda m: m.group(1) + _PUNCTUATION_MAP.get(m.group(2), m.group(2)),
                protected_line,
            )
            protected_line = _RE_PUNC_QUOTE.sub(
                lambda m: _PUNCTUATION_MAP.get(m.group(1), m.group(1)) + m.group(2),
                protected_line,
            )
...
def process_file(file_pth, beg: int = 0):
...
if __name__ == "__main__":
    input_file = (
        Path(__file__).parent.parent / "文档校正" / "未央河月：最终的编曲 虎山.md"
    )
